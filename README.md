...
Docker Hub 公开搜索无需额外配置。GHCR 浏览按用户或组织范围调用 GitHub Packages API；如需使用，
请只在启动服务的本机环境中设置只读 Token，页面不会接收或显示 Token：

```bash
export CONTAINER_GUI_GITHUB_TOKEN='本机安全输入的只读 Token'
swift run ContainerGUI
```
