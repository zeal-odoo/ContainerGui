...
## Decision 15: 镜像删除保护引用关系和 Apple 系统镜像

**Decision**: 本机镜像行只对未被任何容器引用且不是 Apple `vminit` 的目标显示“删除镜像”。提交携带
规范名称、相同的确认目标和幂等键；后端在入队和 CLI 执行前均读取镜像及容器列表，固定执行
`image delete <name>`，不提供 `--all` 或 `--force`，并以删除后完整镜像列表中的目标缺失完成 Operation。

**Rationale**: 停止的容器仍依赖其镜像，Apple `vminit` 是容器运行时的系统资源；只按容器运行状态或
CLI 退出码判断都会扩大不可恢复操作的风险。Docker Hub 短名和规范名称统一比较，避免别名绕过保护。

**Alternatives considered**:

- 自动删除引用镜像的容器：把多个不可恢复动作合并，拒绝。
- 暴露 `--all` 或 `--force`：目标范围过大且无法在页面精确确认，拒绝。
- 允许删除 `vminit` 后按需重拉：会影响运行时可用性且没有用户价值，拒绝。
