...
## Follow-up: v2.23.1 大量日志响应性修复

- [x] T014 用合成大批量日志复现 AI 控制响应和浏览器逐片段重绘问题，先确认回归测试失败。
- [x] T015 限制需脱敏的最近日志窗口，将准备工作移出 AI 控制 actor；保持隐私与取消边界。
- [x] T016 合并实时日志的 DOM 更新、限制最近日志显示大小，保持停止/切换容器时不串日志。
- [x] T017 完整测试、release 构建、浏览器压力验证，版本递增至2.23.1，记录结果并仅本地部署/提交。
